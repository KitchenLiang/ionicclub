    ...
    $scope.login = function() {
      if ($scope.processing) {
        ...
        var loginPopup = $ionicPopup.show({
            template: '<input class="loginuser" type="text" ng-model="data.loginname" placeholder="用户名"><input class="loginuser" type="password" ng-model="data.password" placeholder="密码">',
            title: '用户登陆',
            subTitle: '',
            scope: $scope,
            buttons: [{
                ...
                onTap: function(e) {
                    ...
                    if ($scope.data.loginname) {
                        User.login($scope.data).$promise.then(function(response) {
                            loginPopup.close();
...
/*      if (window.cordova && window.cordova.plugins.barcodeScanner) {
        ...
        if (ENV.debug) {
          ...
          // show login popup if no barcodeScanner in pc browser
          var loginPopup = $ionicPopup.show({
            template: '<input class="loginuser" type="text" ng-model="data.loginname" placeholder="用户名"><input class="loginuser" type="text" ng-model="data.password" placeholder="密码">',
            ...
            buttons: [{
              ...
              onTap: function(e) {
                e.preventDefault();
                if ($scope.data.loginname) {
                  User.login($scope.data.loginname).$promise.then(function(response) {
                    loginPopup.close();
                    loginCallback(response);
                  }, $rootScope.requestErrorHandler());
                }
...
.factory('User', function(ENV, $resource, $log, $q, Storage, Push,My,$ionicLoading) {
    var storageKey = 'user';
    var resource = $resource(ENV.api + '/login');
    var userResource = $resource(ENV.api + '/user/:loginname', {
      loginname: ''
    });
    var user = Storage.get(storageKey) || {};
    return {
      login: function(authordata) {
        var $this = this;
        return resource.save({
            loginname: authordata.loginname
        }, authordata, function(response) {
          $log.debug('post accesstoken:', response);
          // console.log(response.data)
            if (response.code == 200) {
                $this.getByLoginName(response.data.loginname).$promise.then(function (r) {
                    user = r.data;
                ...
                user.loginname = response.data.loginname;
